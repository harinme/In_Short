...
export async function createVoiceConversation() {
  const response = await fetch('/api/voice-conversations', { method: 'POST' })
...
export async function transcribeAudio(conversationId: string, audio: Blob) {
  ...
  const response = await fetch(`/api/voice-conversations/${conversationId}/transcriptions`, { method: 'POST', body: formData })
  if (!response.ok) throw new Error(await readError(response))
  return response.json() as Promise<TranscriptionResponse>
}

export async function deleteVoiceConversation(conversationId: string) {
  const response = await fetch(`/api/voice-conversations/${conversationId}`, { method: 'DELETE' })
